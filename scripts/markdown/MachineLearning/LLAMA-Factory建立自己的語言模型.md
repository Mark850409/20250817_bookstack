...
>[!NOTE] 小提示
>使用這個base image `nvcr.io/nvidia/pytorch:24.01-py3` 打包大約139GB左右，建議改採用下方使用`python:3.10-slim`輕量版本為主

使用 `nvcr.io/nvidia/pytorch:24.01-py3` 撰寫`Dockerfile`
...
使用 `python:3.10-slim` 撰寫`Dockerfile`

```Dockerfile
# 使用官方的 Python 3.10 slim 基礎映像
...
# 安裝 bitsandbytes for 4-bit inference
RUN pip install bitsandbytes

# 確保 src 目錄存在並且包含 webui.py
WORKDIR /app/LLaMA-Factory/src
...
services:
  llama-factory:
    ...
    volumes:
      - C:\WorkSpace\docker-llamafactory\huggingface:/root/.cache/huggingface/
      - C:\WorkSpace\docker-llamafactory\data:/app/data
      - C:\WorkSpace\docker-llamafactory\output:/app/output
      - C:\WorkSpace\docker-llamafactory\models:/app/models # 映射自己的models目录
      - C:\WorkSpace\docker-llamafactory:/app # 映射自己的models目录
    environment:
...
![](https://markweb.idv.tw/uploads/202408230014428.png)
